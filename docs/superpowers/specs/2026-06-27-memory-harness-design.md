...
`CRUCIBLE_MEMORY_ENABLED` is truthy.
...
`CRUCIBLE_MEMORY_ENABLED` (default off) is the master switch — when off, `MemoryHarness` is a
no-op pass-through and the loops behave byte-identically to today. This lets us land each phase dark
and enable per-workspace (same pattern as `CRUCIBLE_CHAT_CONTROLLER`, `CRUCIBLE_TASK_SUBSYSTEM`).
...
- **Seam already left:** `RecallEngine.recall()` does `top-3k → final-k`. The reranker slots at that
  `→`. No data-model change; a flagged swap of the rerank function (`CRUCIBLE_MEMORY_RERANKER`, off).
...
CRUCIBLE_MEMORY_WINDOW_TOKENS         # default 128000 — context window size (see P1 open question)
...
# Phase 3
CRUCIBLE_MEMORY_RERANKER              # default off
```
