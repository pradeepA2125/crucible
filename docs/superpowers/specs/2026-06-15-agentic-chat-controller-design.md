...
**Gate plumbing (verified against the Class-A live model).** Interactive gates render from the `/live` poll via `LiveSlot.GateDispatch` keyed by `PendingGate.kind`, *not* from SSE. But `resolve_live_state` derives gates from the active **task's** status/`execution_state.pending_*`, and a controller turn has **no task**. So controller gates live at the **thread** level: `ChatThread.pending_controller_gate: PendingGate` (durable, surfaced by `get_thread_live`), `PendingGate.kind` extended with `mode`/`edit`, plus the in-memory future for the held-open per-edit gate — mirroring how task gates pair `pending_step_review` (for `/live`) with `_pending_step_decisions` (to resume the loop). `propose_mode` (Class-A) sets the `mode` gate + ends the turn; the per-edit gate sets the `edit` gate while awaiting the decision future. Decision routes (`/mode-decision`, `/edit-decision`) clear the gate; resolution writes a breadcrumb. Webview decision posts are handled in `chat-panel.ts::registerHandlers` alongside `stepDecision`.
