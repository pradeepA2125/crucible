...
- **Resumed child is half-visible in chat (follow-up to `95d40ac`, user-verified 2026-06-13):** the fix attaches the thread (gates render, breadcrumbs land, status chip tracks the child) but the resumed run is otherwise mute: no `task_card` for the child id in the transcript, no workbar/"step 1 of 3" execution progress, no live tool pills. Root cause: those all come from the chat-turn SSE stream the extension consumes inside `sendChatMessage` (and from `_write_chat_tool_events` keyed to a chat-created task) — nobody subscribes to a resumed child's task channel. Fix sketch: on `↻ Resumed` (or when `/live` reports an active task id the controller isn't streaming), have the controller attach to the child's `/stream-patch` SSE and forward `tool_call`/`step_started` etc. through the same render path as a chat turn; optionally persist a `task_card` for the child in the resume route. Belongs with the durable-telemetry design (Tier B item 3) if not done sooner.
