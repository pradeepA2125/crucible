...
**Task Narrative — IMPLEMENTED 2026-06-14 (inline, TDD, all 10 plan tasks + repair-path completeness add).** 14 commits, branch now 86 ahead of `main`. Backend: `RunEvent`/`TaskNarrative` models; append-only `execution_state.run_events` (step_done/step_failed/replan, appended in `_execute_plan` at the step-complete/exhausted/repair/replan sites — replan BEFORE `_apply_revision`; NEVER pruned); FREE per-step note via `step_summary` on the `verify_done` action → `VerifyResult` → `StepRunResult` → event note; `ReasoningEngine.summarize_run` (impl in `reasoning/engine.py`+`narrative_prompts.py`, scripted `run_narrative` kwarg); `_finalize_task_narrative` in the `_execute_plan` finally (READY_FOR_REVIEW outcome=succeeded + FAILED/ABORTED), **best-effort try/except** (engine without summarize_run never fails the task). Exposure via `resolve_live_state`+`TaskResult`/`TaskView`; `_write_chat_narrative` persists a durable transcript msg; `_find_recent_task` adds it for the classifier. Frontend: `TaskNarrativeSchema` + `toTaskNarrative` mapper; controller forwards to `renderLiveReview`/`renderLiveError`; ReviewCard/ErrorCard render headline+points. **Delta-replan re-trace done** (confirmed `_apply_revision` engine.py:~2185 drops reverted-without-replacement → n--, appends brand-new → n++, removes reverted from completed_step_ids → x--; append-only log immune). Verification: editor-client 28, extension 30, webview 162 green; typecheck+builds clean; pytest = only the 11 known pre-existing. **NOT smoked** (Final Step 3 interactive): run a multi-step task → ReviewCard shows headline+points; force a failure → ErrorCard narrative; delta replan → narrative mentions course-correction; follow-up chat msg → next turn references prior task narrative.

**(historical) Task Narrative — spec + plan were written 2026-06-13 (NOT yet implemented):** Spec `docs/superpowers/specs/2026-06-13-task-narrative-design.md`, plan `docs/superpowers/plans/2026-06-13-task-narrative.md` (10 tasks, TDD, anchored). Idea (user-driven): LLM-authored run narrative (headline + points) for the Review/Error cards AND as next-chat-turn context. Mechanism: **append-only `run_events` log** (per-step prose captured FREE via a new `step_summary` field on the `verify_done` action; deterministic step_failed/replan events) → one `summarize_run` LLM call per outcome (READY_FOR_REVIEW success + FAILED/ABORTED), reusing the Tier B finally chokepoints. Consumed next turn via existing transcript-history + `_find_recent_task` plumbing.
