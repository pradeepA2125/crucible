...
**Interfaces:**
- Produces: `class SessionRegistryFile(path: Path)` — `record(sessions: Iterable) -> None` (rewrites the JSON list `[{session_id, pid, pgid, thread_id, command, executable, started_at}]` from objects with `.session_id/.thread_id/.command_line/.executable/.started_at/.proc.pid/.proc.pgid`), `clear() -> None`, `reap_orphans() -> int` (kill recorded pgids still alive whose `ps -o command=` first-token **basename prefix-matches** the recorded executable's basename, both directions; Unix-only — on win32 just clears the file; always ends by clearing; returns count killed). All methods best-effort: `OSError`/parse errors → `logger.warning`, never raise.
- **IMPLEMENTATION DEVIATION (found during T4, macOS):** the original "recorded command's first token is a substring of the live `ps` line" guard NEVER matches for interpreter sessions on macOS — `ps` shows the symlink-resolved binary (venv `python3.13` → `.../Python.app/Contents/MacOS/Python`), and a workspace path containing a space also breaks first-token splitting. Fix shipped: sessions record `executable` as its own field (`_Session.executable`) and the reap compares lowercase **basenames with bidirectional prefix match** (`python` ↔ `python3.13`). Slightly looser pid-reuse guard — accepted, it's a crash-recovery path (spec wart already noted).
