...
export class RuntimeManager {
  ...
  mcpDisabled(): string[];                           // globalState "crucible.mcpDisabledServers"
  setMcpDisabled(names: string[]): Thenable<void>;
  skillsDisabled(): string[];                        // globalState "crucible.skillsDisabled"
...
- Secrets: `context.secrets.store("crucible.providerKey." + backend, key)`; provider
  backend+model in `globalState` keys `crucible.provider.backend` / `crucible.provider.model`.
...
`crucible.runSetup` (opens the Task 12 wizard), `crucible.restartBackend`,
`crucible.openSettingsPanel` (Task 13). On activation with a workspace folder:
if `!manager.isInstalled()` → run `crucible.runSetup`; else
`manager.startForWorkspace(folder)` and point the existing backend-URL plumbing
(`crucible.backendBaseUrl` consumers / `controller.ts` client construction) at
`manager.backendUrl(folder)` when the setting is at its default — an explicit
user-set `crucible.backendBaseUrl` (the dev flow) always wins and skips managed
spawn entirely. Status bar item: `$(rocket) Crucible: starting…` → `✓ :<port>` →
...
`crucible.policy.shell` → `CRUCIBLE_SHELL_POLICY`, `crucible.policy.scope` →
`CRUCIBLE_SCOPE_POLICY`, `crucible.memory.enabled` → `CRUCIBLE_MEMORY_ENABLED`,
`crucible.memory.reranker` → `CRUCIBLE_MEMORY_RERANKER` (only when the user set
...
- Modify: `apps/vscode-extension/src/extension.ts` (`crucible.runSetup` opens the panel)
...
`rollupOptions.input`. `crucible.runSetup` opens the panel.
...
- Modify: `apps/vscode-extension/src/extension.ts` (`crucible.openSettingsPanel`)
...
interface SettingsState {
  ...
  envFlags: Record<string, string>;   // crucible.policy.shell / .policy.scope / .memory.enabled / .memory.reranker
...
**Interfaces:**
- Produces: `buildMcpEntry(input: { transport: "stdio" | "http" | "sse"; commandLine?: string; url?: string; envVarNames: string[] }): Record<string, unknown>` — pure, unit-tested; the vscode command (`crucible.mcpAddServer`) chains QuickPick(transport) → InputBox(command or URL) → InputBox(name) → InputBox(comma-separated env var names) → `client.upsertMcpServer(name, buildMcpEntry(...), disabled)` → info toast with resulting state. `crucible.mcpListServers`: QuickPick of servers (`$(check)/$(error)` + tool count) → per-server actions (Enable/Disable → same toggle path as Task 13, Reconnect, Remove).
...
`contributes.commands`: `crucible.runSetup` ("Crucible: Run Setup"),
`crucible.openSettingsPanel` ("Crucible: Open Settings"),
`crucible.restartBackend` ("Crucible: Restart Backend"),
`crucible.mcpAddServer` ("Crucible: Add MCP Server"),
`crucible.mcpListServers` ("Crucible: List MCP Servers").
`contributes.configuration`: keep `crucible.backendBaseUrl` (its description now
documents "leave default for the managed backend; set explicitly to attach to a
dev backend"); add `crucible.managedRuntime.enabled` (boolean, default `true` —
kill-switch back to the pure dev flow); add the env-flag settings the panel and
`RuntimeManager` read: `crucible.policy.shell` (`ask`|`allow_all`, default `ask`),
`crucible.policy.scope` (`strict`|`ask`|`auto`, default `ask`),
`crucible.memory.enabled` (boolean, default `false`), `crucible.memory.reranker`
...
- Update `CLAUDE.md` (new backend routes, `CRUCIBLE_PORT`/lockfile, `CRUCIBLE_SKILLS_DISABLED`, managed-runtime overview + the `crucible.managedRuntime.enabled` kill-switch) as the final commit: `docs(claude): P4 managed runtime + settings surfaces`.
