...
- **Backend:** worktree `services/agentd-py` via `scripts/stress/start-backend.sh` with **`CRUCIBLE_CHAT_CONTROLLER=1` exported** before launch, `--workspace <REAL ws OUTSIDE .tmp>` (graph indexing needs a non-`.tmp` ancestor). Port :8001 (workspace `.vscode/settings.json` pins `crucible.backendBaseUrl=http://localhost:8001`).
...
**Still not driven live:** J7 final review→Finish→SUCCEEDED leg (pre-existing Tier-B pipeline).

### 2026-06-16 — Phase J session 3 (cont.) — J11 driven: restart-durable history ✓, but **measured ZERO KV reuse** (finding #13)

Measurement tool: TQP (`/v1/chat/completions`) returns a `timings` block with `prompt_n` (tokens evaluated) + `cache_n` (prefix tokens reused). A direct probe confirmed TQP's prefix cache works **without** `cache_prompt` (two shared-prefix calls: 2nd reused `cache_n=708`). Added a temp `turboquant timings:` log line in the transport to capture the actual agentd-turn numbers (kept — useful observability; uncommitted).

- **✅ Restart-durable history (the core J11 claim) VERIFIED.** Thread `chat-ba1afd750f08` (3-turn history). Restarted agentd twice (uvicorn `--reload`, TQP up) between turns; each follow-up answered correctly from rehydrated history: "we agreed to add round-price to …/util/pricing.py / didn't create a new file because pricing.py already houses related utils" — citing all prior turns. History rehydrates from the DB → `seed_history` across a process restart.
- **✅ Off-the-wire exclusion VERIFIED.** `GET /v1/chat/threads/{id}` returns only the 8 wire fields — NO `controller_conversation_history` / `controller_retrieval_seed`.
- **🔴 Finding #13 (significant, measured) — controller turns get ZERO KV-cache reuse on TQP.** Every controller turn re-prefills the **full ~13k-token prompt** (`prompt_n≈13339–13578, cache_n=0, prompt_ms≈30–33k → ~33s of prompt-eval EVERY turn`). Measured across **3 consecutive same-thread turns with NO restart** (and 2 across restarts) — all `cache_n=0`, so this is NOT a restart issue, it's turn-over-turn. **Root cause (traced to the bytes):** consecutive turns have an **identical system prompt** (`sys_md5` stable) but a **different user-content head** (`head256_md5` differs each turn) — because `ChatController._run_loop` sets `plan_context={"goal": goal, …}` with `goal` = the **current turn's user message**, and `build_controller_step_payload` places `goal` **FIRST** in the payload. The per-turn-varying message sits at the very front of the user content, so the cacheable prefix diverges right after the system block → no reuse. **The J11 byte-identity unit test does NOT catch this** — it compares the *same* turn across a restart (same inputs), never *consecutive* turns (where `goal` changes). Contradicts the controller's stated KV-cache-stability design (stable head → append-only history → per-turn fields LAST). **Fix direction:** keep the current message OUT of the cached head — append it as the latest `conversation_history` entry (where it belongs) and/or move `goal` to the tail with `instruction`; then the only per-turn delta is at the end and the multi-k-token prefix reuses. (Not yet fixed — design touch; needs planner-parity thought + re-measure.)
  - Caveat: `cache_n=0` is even lower than the ~system-length reuse one might expect from an early divergence — consistent with this llama.cpp build only reusing when the divergence is late (the probe diverged at the END → reused 708/1224; the controller diverges right after the system block → ~0). Either way the practical result is measured and clear: ~33s wasted prompt-eval per turn.
  - **✅ FIXED + verified live.** Moved `goal` out of the payload head to the per-turn TAIL (after `conversation_history`, with `instruction`/`budget_status`) in `build_controller_step_payload`. Re-measured two back-to-back turns on TQP: **before fix `cache_n=0, prompt_n≈13.6k, ~33s`; after fix the 2nd turn `cache_n=8192, prompt_n=5483, prompt_ms=14250`** — KV prefix reuse restored, prompt-eval ~halved. Added regression tests in `test_controller_payload.py`: `test_payload_key_order_is_cache_stable` now asserts `conversation_history < goal < instruction`, and `test_goal_change_preserves_cacheable_prefix` proves changing only `goal` leaves every byte through `conversation_history` shared (the turn-over-turn axis the byte-identity test never exercised). 786 passed / 3 skipped. Also added a permanent `turboquant timings:` log line in the transport (`prompt_n`/`cache_n`/`prompt_ms`) so KV reuse stays measurable.
