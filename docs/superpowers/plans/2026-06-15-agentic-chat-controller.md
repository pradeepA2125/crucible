...
**NEXT:** **F** (F0 thread-gate `/live` plumbing + ChatController + gates/routes; **review at end of F**) → **G/H** (flag + invariants + full suite; **review at H2**) → **I** (frontend; **needs `npm install` at repo root first**) → **J** (live smoke — interactive dev-host, needs the human) → **K** (delete legacy explore→classify→route).
