...
- 🚧 **I2 IN PROGRESS (uncommitted WIP — see WIP commit):** DONE: `controller.ts` extracted `streamTurn(stream: AsyncIterable<StreamEvent>)` out of `sendChatMessage` (small head-edit; the existing event loop became the method body; caller sets `setChatInputEnabled(false)`+`turnAbort` then `await this.streamTurn(...)`) — **28 controller tests still GREEN, typecheck GREEN**; added `controller.handleModeDecisionFromChat(threadId, mode)` (streams `postModeDecision` via `streamTurn`) + `handleEditDecisionFromChat(threadId, decision, reason)` (plain `postEditDecision`; continuation rides the held-open message stream); `chat-panel.ts` added `ModeDecisionHandler`/`EditDecisionHandler` type aliases.
  **REMAINING I2 (do these next, in order):**
  1. `chat-panel.ts`: add ctor params `onModeDecision: ModeDecisionHandler`/`onEditDecision: EditDecisionHandler` (after `onStepDecision` param at ~:44); add dispatch in `registerHandlers` after the `stepDecision` branch (~:136): `} else if (m["type"]==="modeDecision"){ p=this.onModeDecision(m["threadId"] as string, m["mode"] as string); } else if (m["type"]==="editDecision"){ const d=m["decision"]==="accept"?"accept":"reject"; p=this.onEditDecision(m["threadId"] as string, d, (m["reason"] as string)??""); }`.
  2. `extension.ts`: in `new ChatPanel(...)` add two lambdas after the `onStepDecision` lambda (~:32-33): `(threadId, mode)=>controller.handleModeDecisionFromChat(threadId, mode)`, `(threadId, decision, reason)=>controller.handleEditDecisionFromChat(threadId, decision, reason)` — **match ctor param ORDER**.
  3. Create `webview-ui/src/components/messages/gates/ModeGate.tsx` (full code in plan Task I2 Step 3 — `CardShell icon="bolt"`, options→`vscode.postMessage({type:"modeDecision",threadId:taskId,mode})`; `taskId` carries the threadId) and `EditGate.tsx` (= copy `StepGate.tsx` verbatim — already read it; swap both `vscode.postMessage({type:"stepDecision",taskId,decision})` → `{type:"editDecision",threadId:taskId,decision:"accept"|"reject",reason}`; reject sends `reason:""` for v1, or add a textarea).
  4. `LiveSlot.tsx` `GateDispatch` switch: add `case "mode": return <ModeGate taskId={taskId} payload={payload} />;` and `case "edit": return <EditGate taskId={taskId} payload={payload} />;` (+ import both).
  5. Tests: `webview-ui/src/test/ModeGate.test.tsx` (plan I2 Step 1 — jsdom, mock `../vscodeApi`, assert sketch renders + click posts `modeDecision`); extend `vscode-extension/test/controller.test.ts` client stub with `postModeDecision`(async generator) + `postEditDecision`(async) and add a test for `handleEditDecisionFromChat`. **MUST `npm run build` (webview-ui dist) before any smoke.**
  6. Verify: `npm run -w @ai-editor/vscode-extension typecheck` + `test`; commit I2 `feat(webview): ModeGate + EditGate in LiveSlot; mode/edit decision wiring`.
...
**NEXT after I2:** **J** (live smoke — interactive dev-host, needs the human; flip `AI_EDITOR_CHAT_CONTROLLER=1`; scenarios J1–J7 in plan) → **K** (delete legacy explore→classify→route at `=0` retirement).
