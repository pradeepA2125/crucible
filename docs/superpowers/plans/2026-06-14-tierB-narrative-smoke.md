...
- **Chat-turn Stop placement (user feedback 2026-06-14):** the streaming-turn Stop is a separate button on the left of the composer footer (`InputArea.tsx`, `availability.showStop` block ~line 177). Conventional pattern is the **Send button itself toggling to Stop** while a turn streams, then back to Send. Move it: drop the standalone Stop, and render the right-hand action as Stop (red, `handleStop`) when `showStop`, else Send. (The task-execution "Stop & keep / Stop & revert" affordance is separate and stays.)
- **Chat abort, shouldn't have resume option:** once abort is selected, execution aborted message should not contain resume button.
- **dismissing aborted:** dismissing aborted live gate gate doesn't add breadcrumb rightaway and add on reload(going back to thread list n back).
