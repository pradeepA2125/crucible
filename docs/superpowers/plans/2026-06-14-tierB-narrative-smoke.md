...
- **Scenario 3 — narrative empty-run_events (`8bcaa87`): ✅ VERIFIED.** `task-bca38c505fcf` aborted (API `/abort {revert}`) at the very start of EXECUTING → `run_events_count=0`, `task_narrative.outcome="aborted"`, headline **"Run stopped before any step completed."** (NOT the old "aborted during initial planning phase"). `failure_summary=null`, `run_summary 0 of 2`.
- **Scenario 4 — Send↔Stop composer toggle (`8bcaa87`): ✅ VERIFIED (fresh build only).** Idle composer = single "Send"; during a streaming chat turn = single "Stop" (no standalone stop button). Stale build had shown both = old behavior.
- **Scenario 5 — aborted ErrorCard hides Resume (`8bcaa87`): ✅ VERIFIED (fresh build only).** After rebuild+reload the aborted card shows **["Re-plan","Dismiss"]**, Resume gone. Also durable across reload: narrative + `✗ Run reverted` breadcrumb persist, no "Execution failed". (Stale build → Resume present = the original deferred bug.)
- **Scenario 2 — `failure_summary.message` populated on FAILED (`8bcaa87`): ⏳ NOT re-verified** — needs a genuine FAILED terminal (non-deterministic; was the qwen3 content-field grind). Left for a forced-failure pass (validation-profile full + a failing goal).

**Infra fixes this session:** `start-backend.sh` reap-guard `set -e` abort (committed `9825444` — watcher never launched on clean env); confirmed backend port must be **:8001** (workspace `.vscode/settings.json` pin overrides the `127.0.0.1:8000` extension default).
