...
Talks to llama-server built from TheTom/llama-cpp-turboquant (feature/turboquant-kv-cache).
The server exposes the standard OpenAI /v1/chat/completions endpoint.
...
Key difference from OllamaJsonTransport:
  ...
  - max_tokens=-1 on JSON calls (unlimited) to prevent thinking from eating the budget
"""
...
class TurboQuantJsonTransport(ModelJsonTransport):
