...
impl IndexerConfig {
    pub fn from_env() -> Self {
        let workspace_root = env::args()
            ...
            .nth(1)
            .map(PathBuf::from)
            .or_else(|| env::var("CRUCIBLE_WORKSPACE").ok().map(PathBuf::from))
            .unwrap_or_else(|| PathBuf::from("."));

        let max_parse_workers = env::var("CRUCIBLE_MAX_PARSE_WORKERS")
            .ok()
            .and_then(|value| value.parse::<usize>().ok())
            .unwrap_or(4);

        let watch_enabled = env::var("CRUCIBLE_WATCH")
            .map(|value| value != "0")
            .unwrap_or(true);

        let lsp_enabled = env::var("CRUCIBLE_LSP_ENABLED")
            .map(|value| value != "0")
            .unwrap_or(true);

        let lsp_ts_cmd = env::var("CRUCIBLE_LSP_TS_CMD")
            .unwrap_or_else(|_| "typescript-language-server --stdio".to_string());
        let lsp_py_cmd = env::var("CRUCIBLE_LSP_PY_CMD")
        ...
        let lsp_startup_timeout_ms = env::var("CRUCIBLE_LSP_STARTUP_TIMEOUT_MS")
            ...
            .unwrap_or(3_000);
        let lsp_request_timeout_ms = env::var("CRUCIBLE_LSP_REQUEST_TIMEOUT_MS")
            .ok()
            .and_then(|value| value.parse::<u64>().ok())
            .unwrap_or(3_000);
        ...
        let backend_url = env::var("CRUCIBLE_BACKEND_URL").ok();
